 ...
 * to create there, and the `default-branch` input behind it is caller YAML,
 * which `pull_request_target` reads from the base branch where a pull request
 * cannot edit it. Push access is this input's provenance and nothing wider —
 * it is not the line `isTrustedAuthor` draws over the world-writable feedback
 * surfaces, which admits org-adjacent or better (#68). Neither is what makes
 * this call safe: the argv form is, and it holds however the ref got here.
 */
