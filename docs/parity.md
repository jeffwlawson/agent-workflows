...
| `agent:follow-ups` | ❌ | ➕ PRs — the marker *and* the manual trigger in one string, disambiguated by event type rather than by a second label a human could choose wrong. Added by review, removed only by a filing run that filed; removing it by hand is the opt-out (`docs/ADOPTING.md` §3) |
| `agent:update-branch` | ✅ | ✅ |
